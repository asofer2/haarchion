...
const html = fs.readFileSync(
  path.join(__dirname, "p-tuvia-tsafir.html"),
  "utf8"
);
...
const birthName = stripTags(birthNameMatch?.[1] || "") || "טוביה קוזלובסקי";
...
console.log(
  "by role",
  Object.fromEntries(
    [...new Set(credits.map((c) => c.role))].map((r) => [
      r,
      credits.filter((c) => c.role === r).length,
    ])
  )
);
console.log("notes", notes.map((n) => `${n.heading}:${n.items.length}`));

fs.writeFileSync(
  path.join(__dirname, "../../src/data/tuvia-tsafir.json"),
  JSON.stringify(person, null, 2),
  "utf8"
);
console.log("wrote src/data/tuvia-tsafir.json");
